PERCENT_PACK = 15
$("#photos").empty()
  .append("<%= escape_javascript(render(partial: 'tables/general_methods/photo/first', layout: 'tables/general_methods/photo/first')) %>")
...
$("#product_type_furniture_id").empty()
  .append("<%= escape_javascript(render(partial: 'tables/general_methods/type_furniture/form', collection: @type_furnitures, as: 'type_furniture')) %>")
...
if $("#number_factor") != []
  $("#number_factor").empty()
    .append("<%= escape_javascript(render(partial: 'tables/general_methods/discount/factor', as: 'discount')) %>")
...
chosen.dropdown.find('input').on 'keyup', (e) ->
  ...
  if e.which == 13 and chosen.dropdown.find('li.no-results').length > 0
    ...
    $.ajax 'tables/general_methods/table_specification/update_articles',
      type: 'GET'
      dataType: 'script'
      data: {
      }
