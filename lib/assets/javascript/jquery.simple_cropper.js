...
(function ($) {
    ...
    $.fn.simpleCropper = function (onComplete) {
        ...
        var scaled_width = 100;
        var scaled_height = 175;
        var x1 = 0;
        var y1 = 0;
        var x2 = 0;
        var y2 = 0;
        ...
        $(document).ready(function () {
            ...
            $('body').on('change','#fileInput',function (click) {
        ...
        function preview() {
            ...
            canvas.width = sw * kw;
            canvas.height = sh * kh;
            context.drawImage(current_image, (x1 * kw), (y1 * kh), (sw * kw), (sh * kh), 0, 0, (sw * kw), (sh * kh));
            ...
            file_display_area.before('<a href="#"></a>');
            file_display_area.prev().append(imageFoo);
            
            if(file_display_area.parent().attr('id') == 'photos') {             
                if($('#product_photo_base64_form')) {
                    var hiddenField = $('#product_photo_base64_form');
                    if(hiddenField.val()) {
                        hiddenField.val(hiddenField.val() + ',' + dataUrl);
                    } else {
                        hiddenField.val(dataUrl);
                    }
                }
            }
            if(file_display_area.parent().attr('id') == 'size_images') {                
                if($('#product_size_image_base64_form')) {
                    var hiddenField = $('#product_size_image_base64_form');
                    if(hiddenField.val()) {
                        hiddenField.val(hiddenField.val() + ',' + dataUrl);
                    } else {
                        hiddenField.val(dataUrl);
